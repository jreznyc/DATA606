...
1.  What type of plot would you use to display the relationship between `runs` 
    ...
    **The relationship does appear to be linear. Yes, I would be comfortable using a linear model to predict the number of runs  **
...
2.  Looking at your plot from the previous exercise, describe the relationship 
    ...
    **The variables appear to have a positive linear relationship, and are strongly correlated. There appear to be outliers for lower values of at_bats ** 
...
3.  Using `plot_ss`, choose a line that does a good job of minimizing the sum of
    ...
    **The smallest sum of squares I got was 132594.2**
...
from the information above, the formula for the linear model is:
\[
...
5.  If a team manager saw the least squares regression line and not the actual 
    ...
    **He would predict ~700 runs, this would be an underestimation by about 25 runs**
...
6.  Is there any apparent pattern in the residuals plot? What does this indicate
    ...
    **The residuals plot appears to be fairly flat overal with slight curves, this would indicate a linear   relationship** 
...
7.  Based on the histogram and the normal probability plot, does the nearly 
    ...
    **Yes they do appear to be nearly normal** 

*Constant variability*:

8.  Based on the plot in (1), does the constant variability condition appear to 
    be met?  
    **Yes, the constant variability conditions appears to be met**
...
-   Choose another traditional variable from `mlb11` that you think might be a 
    ...
    **comparing runs and hits, it appears they do have a linear relationship** 
```{r}
...
-   How does this relationship compare to the relationship between `runs` and 
    ...
    tell?  
    **The $R^2$ of runs ~ hits, is .6419 which is significantly higher than runs ~ at bats, as a result hits are a better predictor of runs** 
```{r}
summary(m3)
```
...
-   Now that you can summarize the linear relationship between two variables, 
    ...
    five).
    **The variable which best predicts runs is batting average, this has an $R^2=65.61\%$** 
    
```{r}
# m <- lm(runs ~ hits, mlb11)
# plot(mlb11$runs ~ mlb11$hits)
# abline(m)
# summary(m)

m <- lm(runs ~ bat_avg, mlb11)
plot(mlb11$runs ~ mlb11$bat_avg)
abline(m)
summary(m)

# m <- lm(runs ~ strikeouts, mlb11)
# plot(mlb11$runs ~ mlb11$strikeouts)
# abline(m)
# summary(m)
# 
# m <- lm(runs ~ stolen_bases, mlb11)
# plot(mlb11$runs ~ mlb11$stolen_bases)
# abline(m)
# summary(m)
# 
# m <- lm(runs ~ wins, mlb11)
...
-   Now examine the three newer variables. These are the statistics used by the 
    ...
    **The newer variables have a much higher correlation to runs, and as such are better predictors of runs. The best variable is on-base plus slugging, with $R^2=93.49\%$
    
```{r}
m <- lm(runs ~ new_onbase, mlb11)
plot(mlb11$runs ~ mlb11$new_onbase)
abline(m)
summary(m)

m <- lm(runs ~ new_obs, mlb11)
plot(mlb11$runs ~ mlb11$new_obs)
abline(m)
summary(m)

m <- lm(runs ~ new_slug, mlb11)
...
-   Check the model diagnostics for the regression model with the variable you 
    ...
    **As seen below, the variable new_obs has a linear relatioship with runs, has nearly normal residuals distribution and consistent variance in residuals** 
...
#Linearity 
plot(m$residuals ~ mlb11$new_obs)
abline(h = 0, lty = 3)  # adds a horizontal dashed line at y = 0
